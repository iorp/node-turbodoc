...
const parseJSJSON = (input, options = {}) => {
  ...
  options = deepMerge({
    ...
    // Performance tests
    'nodeDataKeyName': '@',
    // the subnode name here the node data is stored 
    'maxDepth': null,
    // The max level depth
    'captureBlocks': true,
    // Capture also doc blocks if present,
    'captureRoutes': true,
    // @deprecated markers @todo kill the markers in the future
    'markers': {
      // marker tags in single line comments
      'todo': '@todo' // This tells turbodoc to capture @todo tags after single line comments (//) 
    },
    filter: (node, offset, lines, line, self) => {
      return true;
    },
    // filter: if it returns true ,the node will be stored, if not ,the collected object will not be stored. Do node.children for children, 
    onBeforeStoreNode: (collected, offset, lines, line, self) => {
      return collected;
    },
    // This allows to modify the collected node data before being stored. 
    onAfterStoreNode: (collected, offset, lines, line, self) => {
      return collected;
    },
  ...
  // extract function
  function extract(input = null, attributes = {}, level = 0, parent = null) {
    // Note use only input parameter, the others are internal
    ...
    while (attributes.offset < input.length) {
      ...
      // apply filter
      if (collected && typeof options.filter === 'function' && !options.filter({
        ...collected[options.nodeDataKeyName],
        children: odel(collect, '@')
      }, attributes.offset, lines, line, this)) collected = null;
      if (collected != null) {
        // extraNodeAttributes

...
const parseFile = (options, {
  ...
  if (isDirectory) {
    ...
    const globalFileTreePath = nodeOutputFilePath.substring(docPath.length).replace(/\./, '').split('\\').filter(e => e !== "");
    ...
    modelOptions.onAfterStoreNode = (collected, offset, lines, line, self) => {
      if (collected['@']) {
        collected['@'] = {
          ...
          route: [...filePath.slice(0, filePath.lastIndexOf('.')).substring(targetPath.length).split('\\').filter(element => element !== ""), ...collected['@'].route]
        };
      }
    ...
    if (makeLocal) {
      ...
      renders.forEach(element => {
        ...
        if (formatterOptions.template) {
          ...
          const map = readFile(path.join(docPath, 'map.json'), {
            datatype: 'JSON'
          });
          if (map.error) {
          ...
          resultContents = formatterOptions.template.layout({
            map: map.data,
            root,
            main: resultContents,
            pluginOptions
    ...
    if (makeGlobal) {
      ...
      if (globalMode === 'ram') {
        ...
        const read = readFile(globalPath, {
          datatype: 'JSON'
        });
        if (read.error) {
          console.error(read);
        ...
        const written = writeFile(globalPath, newTree, {
          datatype: 'JSON'
        });
        if (written.error) {

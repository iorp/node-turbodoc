...
const extractDoc = (options = {}) => {
  ...
  if (!targetPath) {
    console.error('extractDoc: targetPath is required');
    process.exit(1);
  }
  if (!docPath) {
    console.error('extractDoc: docPath is required');
    process.exit(1);
  }
  var response = {
    operations: []
  ...
  const runOperations = (filePath, level, isDirectory) => {
    ...
    operations.forEach(operation => {
      ...
      response.operations.push(method(args, {
        filePath,
        level,
        isDirectory
      }, options, data));
  ...
  if (targetPath === docPath) return {
    error: true,
    src: 'turbodoc extractDoc',
    code: 'wrongOutput',
    exception: '"targetPath" and "docPath" cannot be the same.'
  };
  if (!directoryExists(targetPath)) return {
    error: true,
    src: 'turbodoc extractDoc',
    code: 'wrongTarget',
    exception: '"targetPath" must be a valid directory path."' + targetPath + '" is not.'
  };
  if (!directoryExists(docPath)) makeDirectory(docPath);
  ...
  if (data.globals.parsedMap) {
    ...
    if (makeGlobalJson.error) {
      console.error('extractDoc makeGlobalJson error.');
      process.exit(1);
    }
    // js
    ...
    if (makeGlobalJs.error) {
      console.error('extractDoc makeGlobalJs error.');
      process.exit(1);
    }
